// Nature Gallery - S3 Integration
document.addEventListener('DOMContentLoaded', function () {
    const gallery = document.getElementById('natureGallery');

    if (!gallery) {
        console.error('Nature gallery element not found!');
        return;
    }

    // S3 URL format (same as wedding gallery)
    const S3_BUCKET_URL = 'https://s3.eu-north-1.amazonaws.com/pelechmedia.co.uk';

    // Generate image URLs for nature photos (27 images)
    const natureImageFilenames = [
        'nature 1.jpg',
        'nature 2.jpg',
        'nature 3.jpg',
        'nature 4.jpg',
    ...
    const natureImages = natureImageFilenames.map(filename =>
        `${S3_BUCKET_URL}/images/nature/${filename}`
    );

    console.log('Generated', natureImages.length, 'nature image URLs');
    ...
    gallery.innerHTML = `
        <div class="loading-state">
            ...
            <p>Loading ${natureImages.length} nature photos...</p>
        </div>
    `;
    ...
    function createGallery() {
        ...
        const imagesToShow = natureImages.slice(0, initialBatch);
        ...
        if (natureImages.length > initialBatch) {
            setTimeout(() => {
                natureImages.slice(initialBatch).forEach((imageSrc, index) => {
    ...
    function createImageItem(imageSrc, index) {
        ...
        item.addEventListener('click', () => openLightbox(index, natureImages));
        ...
        const imageObserver = new IntersectionObserver((entries, observer) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    ...
                    if (currentIndex < natureImages.length - 3) {
                        setTimeout(() => {
                            for (let i = 1; i <= 3; i++) {
                                if (currentIndex + i < natureImages.length) {
                                    const preloadImg = new Image();
                                    preloadImg.src = natureImages[currentIndex + i];
                                }
                            }
                        }, 500);
                    }
        ...
        lightbox.innerHTML = `
            <div class="lightbox-content">
                ...
                <div class="lightbox-counter">
                    <span id="current-image-number">1</span> / <span id="total-images">${natureImages.length}</span>
                </div>
            </div>
        `;
    ...
    function updateLightboxImage() {
        ...
        lightboxImg.alt = `Nature Photo ${imageNumber}`;
...
// Styles (same as wedding gallery)
